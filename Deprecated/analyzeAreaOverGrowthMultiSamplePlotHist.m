%clear all
close all
clear rposition meanArea stdArea meanMit stdMit
...
%% Analysis
meanMit(meanArea == 0) = NaN;
meanArea (meanArea == 0) = NaN;
stdArea2 = squeeze(nanstd(meanArea,1));
meanArea = squeeze(nanmean(meanArea,1));
stdMit = squeeze(nanstd(meanMit,1));
meanMit = squeeze(nanmean(meanMit,1));
